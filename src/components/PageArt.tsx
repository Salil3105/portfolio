...
const ART: Record<ArtKind, React.ReactNode> = {
  ...
  projects: (
    <svg className="art-svg" viewBox="0 0 420 320" role="img" aria-label="Floating project cards">
      ...
      <ellipse cx="212" cy="288" rx="130" ry="16" fill="#000" opacity=".5" />
      <g transform="rotate(-8 150 150)">
        <rect x="58" y="96" width="150" height="112" rx="12" fill="url(#pa-card)" stroke="rgba(255,255,255,.12)" />
        <rect x="70" y="112" width="60" height="6" rx="3" fill="rgba(255,255,255,.18)" />
        <rect x="70" y="126" width="92" height="6" rx="3" fill="rgba(255,255,255,.10)" />
      </g>
      <g transform="rotate(6 300 150)">
        <rect x="226" y="88" width="150" height="112" rx="12" fill="url(#pa-card)" stroke="rgba(255,255,255,.12)" />
        <rect x="238" y="104" width="54" height="6" rx="3" fill="rgba(147,197,253,.75)" />
        <rect x="238" y="118" width="88" height="6" rx="3" fill="rgba(255,255,255,.10)" />
      </g>
      <g>
        <rect x="130" y="130" width="170" height="126" rx="14" fill="rgba(20,20,26,.92)" stroke="rgba(255,255,255,.2)" />
        <circle cx="146" cy="148" r="3.4" fill="#31313a" />
        <circle cx="158" cy="148" r="3.4" fill="#31313a" />
        <circle cx="170" cy="148" r="3.4" fill="#31313a" />
        <rect x="146" y="166" width="74" height="7" rx="3.5" fill="rgba(196,181,253,.95)" />
        <rect x="146" y="182" width="120" height="7" rx="3.5" fill="rgba(255,255,255,.2)" />
        <rect x="146" y="198" width="92" height="7" rx="3.5" fill="rgba(147,197,253,.75)" />
  ...
  experience: (
    <svg className="art-svg" viewBox="0 0 420 320" role="img" aria-label="Rising steps">
      ...
      <ellipse cx="215" cy="290" rx="150" ry="16" fill="#000" opacity=".5" />
      {[0, 1, 2, 3].map((i) => {
        const w = 74, h = 30 + i * 34, x = 58 + i * 78, y = 262 - h;
        return (
          <g key={i}>
            <polygon points={`${x},${y} ${x + w},${y - 16} ${x + w},${y + h - 16} ${x},${y + h}`} fill="url(#pa-glass)" stroke="rgba(255,255,255,.16)" />
            <polygon points={`${x},${y} ${x + w},${y - 16} ${x + w - 14},${y - 22} ${x - 14},${y - 6}`} fill="rgba(255,255,255,.10)" stroke="rgba(255,255,255,.16)" />
          </g>
        );
      })}
  ...
  contact: (
    <svg className="art-svg" viewBox="0 0 420 320" role="img" aria-label="Message and paper plane">
      ...
      <ellipse cx="200" cy="286" rx="126" ry="15" fill="#000" opacity=".5" />
      <g transform="rotate(-5 200 175)">
        <rect x="92" y="118" width="216" height="130" rx="16" fill="url(#pa-card)" stroke="rgba(255,255,255,.16)" />
        <path d="M92 136 L200 200 L308 136" fill="none" stroke="rgba(196,181,253,.8)" strokeWidth="3" strokeLinejoin="round" />
        <rect x="112" y="214" width="78" height="7" rx="3.5" fill="rgba(255,255,255,.14)" />
      </g>
      <g transform="rotate(12 318 92)">
        <path d="M282 92 L358 62 L330 128 L318 100 Z" fill="rgba(124,92,255,.9)" />
        <path d="M318 100 L358 62" fill="none" stroke="rgba(255,255,255,.5)" strokeWidth="2" />
      </g>
  ...
  about: (
    <svg className="art-svg" viewBox="0 0 420 320" role="img" aria-label="Stacked service layers">
      {defs}
      <ellipse cx="210" cy="160" rx="180" ry="126" fill="url(#pa-glow)" />
      <ellipse cx="210" cy="286" rx="136" ry="15" fill="#000" opacity=".5" />
      {[0, 1, 2].map((i) => {
        const y = 210 - i * 56;
        return (
          <g key={i}>
            <polygon points={`210,${y - 34} 330,${y} 210,${y + 34} 90,${y}`} fill="url(#pa-glass)" stroke="rgba(255,255,255,.18)" />
